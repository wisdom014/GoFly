...
function Reviews({ reviews = temporaryReviews }) {
	...
	function handlePointerUp(event) {
		...
		if (Math.abs(distance) > 45) setActiveIndex((current) => Math.max(0, current + (distance < 0 ? 1 : -1)))
		swipeStart.current = null
		event.currentTarget.releasePointerCapture?.(event.pointerId)
	}

	return <section className="reviews-section" aria-labelledby="reviews-title"><div className="reviews-heading"><h2 id="reviews-title">Hear It from Travelers</h2><p>We go beyond just booking trips—we create unforgettable travel experiences that match your dreams!</p></div><div className="reviews-carousel" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} onPointerCancel={() => { swipeStart.current = null }}><div className="reviews-track" style={{ transform: `translateX(-${activeIndex * 100}%)` }}>{slides.map((review, index) => <div className="review-slide" key={`${review.name}-${index}`}><ReviewCard review={review} /></div>)}</div></div><div className="review-platforms"><div><strong>◉ Tripadvisor</strong><span>Reviews <b>●●●●◐</b></span></div><div><strong>★ Trustpilot</strong><span><em>4.5</em> <b>★★★★★</b> Reviews</span></div></div></section>
}
